...
class XUploadAction extends CAction {
    ...
    protected function handleDeleting()
    {
        if (isset($_GET["_method"]) && $_GET["_method"] == "delete") {
			$idImage = isset($_GET['idImage']) ? $_GET['idImage'] : 0;
            $success = false;
            if ($_GET["file"][0] !== '.' && Yii::app()->user->hasState($this->stateVariable)) {
                ...
                if ($this->fileExists($userFiles[$_GET["file"]])) {
                    ...
                    if ($success) {
						Images::model()->deleteByPk($idImage);
    ...
    {
		$albumId = $_POST['XUploadForm']['album_id'];
        $this->init();
        ...
        if ($model->{$this->fileAttribute} !== null) {
            ...
            if ($model->validate()) {
				...
				$modelImage->name = $model->{$this->fileAttribute}->getName();
				
				$imageType = explode('.',$modelImage->name);
				$imageType = $imageType[count($imageType)-1];
				
				$modelImage->image = md5(uniqid()).'.'.$imageType;
				$modelImage->description = '';
				$modelImage->album_id = $albumId;
				...
				$model->{$this->fileNameAttribute} = $modelImage->image;
            ...
            throw new CHttpException(500, "Không thể upload file");
        }
    }
