...
class UserRecoveryForm extends CFormModel {
	...
	public function rules()
	{
		return array(
			...
			array('login_or_email', 'required', 'message'=>'Tên đăng nhập hoặc email không được trống'),
			array('login_or_email', 'match', 'pattern' => '/^[A-Za-z0-9@.-\s,]+$/u','message' => UserModule::t("Tên đăng nhập hoặc email không hợp lệ.")),
	...
	{
		return array(
			'login_or_email'=>UserModule::t("Tên đăng nhập hoặc email"),
		);
	}
	...
	public function checkexists($attribute,$params) {
		if(!$this->hasErrors())  // we only want to authenticate when no input errors
		{
			...
			if($user===null)
				if (strpos($this->login_or_email,"@")) {
					$this->addError("login_or_email",UserModule::t("Email không tồn tại."));
				} else {
					$this->addError("login_or_email",UserModule::t("Tên đăng nhập không tồn tại."));
				}
		}
	}
