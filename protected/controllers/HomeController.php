...
class HomeController extends Controller
{
	...
	{
		$this->pageTitle = "Trang chủ - Mẫu giáo Bảo Ngọc";
		...
		$cri1->addCondition("category_news_id = 1");
		$cri1->order = "id DESC";
		$cri1->limit = 4;
		$arrNoti = News::model()->findAll($cri1);
		
		// Goc dinh duong
		$cri2 = new CDBCriteria;
		$cri2->addCondition("category_news_id = 4");
		$cri2->order = "id DESC";
		$cri2->limit = 4;
		$arrNutrition = News::model()->findAll($cri2);
		
		// tin tuc moi nhat
		$cri3 = new CDBCriteria;
		$cri3->addCondition("category_news_id = 2");
		$cri3->order = "id DESC";
		$cri3->limit = 4;
		$arrNew = News::model()->findAll($cri3);
